...
class PredMsg:
    module: str             # "A" | "B" | "C"
    frame_idx: int          # debe coincidir con el tick
    ts: float               # time.time()
    label: str              # p.ej. "ALTO" | "REPETITIVO" | "inseguro"
    conf: float             # 0..1 (si "inseguro", usar 0.0)
    quality: float          # 0..1
...
# NUEVO: mensaje que emite el fusionador por tick hacia el reporter
@dataclass
class FusionTickMsg:
    frame_idx: int
    ts: float
    level: Optional[int]     # 0..3 o None si "inseguro"
    state: str               # "init"|"estable"|"sube"|"baja"|"espera"|"inseguro"
    target: Optional[int] = None  # target calculado ese tick (para debug)
