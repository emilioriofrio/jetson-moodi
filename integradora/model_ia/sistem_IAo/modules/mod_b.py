# modules/mod_b.py
# --- Variables de entorno ANTES de importar TF (compat Keras2) ---
import os
...
# ======== helpers de tu módulo B original ========
...
def run_worker_B(cfg_path: str, in_q: Queue, out_q: Queue, stop_event: Event):
    ...
    VIS_THR_POSE   = 0.5  # igual que tu código

    print(f"[B] Cargando modelo: {MODEL_PATH}", flush=True)
    print(f"[B] Labels map:     {LABELS_MAP_PATH}", flush=True)

    # --- CARGA MODELO (intento estándar; ya forzamos modo legacy con env vars) ---
    model = None
    load_errors = []
    try:
    ...
    try:
        while not stop_event.is_set():
            ...
            # ¿Es tick? → solo en tick calculamos la predicción
            if (frame_idx % tick == 0) and (len(seq) == N_FRAMES):
                ...
                # Actualizamos estado “last_*” SIEMPRE (aunque salga inseguro)
            ...
            if frame_idx % tick == 0:
                ...
                if qpose < QUALITY_THR:
                    ...
                    if (frame_idx - last_pred_frame) > STALE_FRAMES:
                        ...
                        # 3) Último resultado (puede ser inseguro)
        ...
        try:
            holistic.close()  # <- cierre explícito de MediaPipe
        except Exception:
            pass
