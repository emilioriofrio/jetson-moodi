...
# Desactiva XLA y TensorRT en TF
...
def _mtcnn_detect(detector, frame_bgr, max_side, roi=None):
    ...
    t_ms = (time.time() - t0) * 1000.0
    if not dets:
        print(f"[A][MTCNN] 0 dets | patch={rgb.shape[1]}x{rgb.shape[0]} | scale={scale:.3f} | roi={roi is not None} | {t_ms:.1f}ms", flush=True)
        return None
...
def run_worker_A(cfg_path: str, in_q: Queue, out_q: Queue, stop_event: Event):
    ...
    ROI_ONLY_TICKS = int(cfg.get("modA_roi_track_ticks", 5))  #post. detectar rostro, n ticks buscando en ROI

    # Orden de backends
    cfg_order = cfg.get("modA_backends_order", ["mtcnn"])
        ...
        while not stop_event.is_set():
            ...
            for backend in order:
                try:
                    ...
                    if backend == "mtcnn" and mtcnn is not None:
                        # Inicializar (evita UnboundLocalError)
                        tried_roi = False
                        x0 = y0 = w0 = h0 = None
                        t_det = 0.0
                        ...
                        if (not tried_roi) or (w0 is None or h0 is None or w0 <= 0 or h0 <= 0):
                            ...
                            if hit:
                                ...
                                used_backend = None  # pasar a siguiente
            ...
            if not present:
                ...
                if missing_ticks >= face_timeout_ticks:
                    last_box = None  # olvidar ROI si hay perd.
