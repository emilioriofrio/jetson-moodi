...
ks=list(m.jetson_gpio_data.keys())
pref=[k for k in ks if re.search(r"Orin\\s*Nano",k,re.I)]
...
print(f"[{tkey}] BOARD {PIN} — pull-down externo (10k a GND). PRESIONADO=HIGH (1). Ctrl+C para salir.")
last=GPIO.input(PIN)
print("Inicial:", last, "(0=GND por la 10k, 1=3V3 por el botón)")
try:
    while True:
        v=GPIO.input(PIN)
        if v!=last:
            print("PRESIONADO" if v==1 else "SUELTO")
            last=v
        time.sleep(0.003)
